document.addEventListener('DOMContentLoaded', function() {
  ...
  function fetchAndType() {
    fetchStatsEntries().then(function(data) {
      var terminalText = document.getElementById("terminal-text");
      var firstHalf = 'Overall Stats:\n\n'+'> Total Donations:  ₹ ' + data.total_donations + '/-\n > Total Campaigns: ' + data.total_campaigns +'\n > Total Contributors: ' + data.total_donors;
      var secondHalf = 'Current Month Stats:\n\n'+'> Total Donations:  ₹ ' + data.total_donation_this_month + '/-\n > Total Campaigns: ' + data.total_campaigns_this_month +'\n > Total Contributors: ' + data.total_donors_this_month;
  ...
  function fetchStatsEntries() {
    return fetch('http://localhost/NectarOfService/assets/terminal_stats/get_terminal_stats_data.php')
      .then(response => {
        if (!response.ok) {
