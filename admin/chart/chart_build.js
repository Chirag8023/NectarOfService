 ...
 async function fetchDataAndCreateCharts() {
    ...
    const totalAmountResponse = await fetch('http://localhost/nectarofservice/admin/chart/chart_amount_data.php');
    const totalAmountData = await totalAmountResponse.json();

    // Fetch donations by campaign lst 30 days
    const campaignsResponse = await fetch('http://localhost/nectarofservice/admin/chart/chart_campaigns_data.php');
    const campaignsData = await campaignsResponse.json();

    // Fetch people donated by date
    const donationsByDateResponse = await fetch('http://localhost/nectarofservice/admin/chart/chart_people_data.php');
    ...
    const donationsByDateCtx = document.getElementById('donationsByDateChart').getContext('2d');
